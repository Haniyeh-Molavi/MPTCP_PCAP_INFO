...
def parse_remove_addr_packet(packet_bytes: bytes):
    """Parse MP_REMOVE_ADDR option data from a TCP packet if present."""
    ...
    while i < len(options):
        ...
        if kind == 30:
            ...
            address_ids = [int(byte) for byte in value[1:]] if len(value) > 1 else []
            if not address_ids:
                break
...
def extract_remove_addr_to_csv(
    ...
    if output_csv_path is None:
        csv_path = pcap_path.with_name(f"{pcap_path.stem}_remove_addr.csv")
    else:
        output_csv_path = Path(output_csv_path).expanduser().resolve()
        if output_csv_path.is_dir():
            csv_path = output_csv_path / f"{pcap_path.stem}_remove_addr.csv"
        else:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Extract MP_REMOVE_ADDR features from PCAP files into CSV."
    )
